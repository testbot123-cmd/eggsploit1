...
function rendercart() {
  ...
  cartlist.querySelectorAll('.cartremove').forEach(function(btn) {
    btn.addEventListener('click', function() {
      cart.splice(parseInt(this.getAttribute('data-i')), 1)
      rendercart()
    })
  })
...
submitorder.addEventListener('click', function() {
  ...
  if (!name) {
    ...
    ordermsg.textContent = 'enter your name'
    return
  }
  if (!email || email.indexOf('@') === -1) {
    ordermsg.style.color = '#e74c3c'
    ordermsg.textContent = 'enter a valid email'
    return
  }
  if (cart.length === 0) {
    ordermsg.style.color = '#e74c3c'
...
document.addEventListener('keydown', function(e) {
  if (e.keyCode === konami[kpos]) {
    ...
    if (kpos === konami.length) {
      ...
      console.log('%cachievementunlocked{konami_coffee}--brokenlightmode', 'color: #e8a87c; font-size: 14px; font-weight: bold')
